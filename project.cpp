...
bool secondPass = 0;		// == 0 run first pass, == 1 run 2nd pass
...
void ForkCode(int Layer, int numNeurons, int wr_open, fstream &infile, int backpass_wr_open)
{
    ...
    while(!infile.eof())
    {
        ...
        if( (singleLine2[0] >=65 && singleLine2[0] <=90) || (singleLine2[0] >=97 && singleLine2[0] <=122) )			// concatenation
        {
    ...
    if(test1 == false)
    {
	    ...
	    // if first pass then do back propagation
    ...
    // create new pipe for reading from next layer
    ...
    if(secondPass == 0)
    {
		...
		backpasspipe = mkfifo(tempstr.c_str(), 0777);

		backpass_wr_open_next = open(tempstr.c_str(), O_RDWR);
    }

    // after reading from next layer write backpass_wr_open
...
int main()
{
	...
	int pipd = mkfifo("pipd", 0777);

	if(pipd < 0)
	{
		
		unlink("pipd");
		pipd = mkfifo("pipd", 0777);
	}
	...
	int backpasspipe = mkfifo(tempstr.c_str(), 0777);
	...
	if(pid >= 0)
	{
		if(pid == 0)
		{
		    ForkCode(layer, numNeurons, wr_open, infile, backpass_wr_open);		// numNeurons == no of inputs in pipe
		}
		else
		{
			char token2[100] = {'\0'};
			...
			// extract input values from token2 and add to pipe
